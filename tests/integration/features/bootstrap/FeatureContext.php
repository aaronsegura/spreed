...
class FeatureContext implements Context, SnippetAcceptingContext {
	...
	/**
	 * @Given user :user set the expire date to :expireDate of room :identifier with :statusCode (:apiVersion)
	 */
	public function userSetTheExpireDateToWith(string $user, int $expireDate, string $identifier, int $statusCode, string $apiVersion = 'v4'): void {
		$this->setCurrentUser($user);
	...
	/**
	 * @Given user :user check if expire date of room :identifier is :expireDate (:apiVersion)
	 */
	public function userCheckIfExpireDateOfRoomIsX(string $user, string $identifier, int $expireDate, string $apiVersion = 'v4') {
		$this->setCurrentUser($user);
		$this->sendRequest('GET', '/apps/spreed/api/' . $apiVersion . '/room/' . self::$identifierToToken[$identifier]);
		$room = $this->getDataFromResponse($this->response);

		Assert::assertEquals($expireDate, $room['expireDate']);
	}
